...
class scene3(Scene):
    def construct(self):
        ...
        rectangle = RoundedRectangle(stroke_width = 8, stroke_color = WHITE,
                    fill_color = BLUE, width = 4.5, height = 2).shift(UP*3 + LEFT*4)
...
class scene4(Scene):
    def construct(self):
        ...
        line_circumference = always_redraw(lambda : Line(start = circle.get_left(),
                                           end = circle.get_right(), stroke_color = YELLOW, 
                                           stroke_width = 5).set_length(2 * r.get_value() * PI
                                           ).next_to(circle, DOWN, buff = 0.2))
        
        triangle = always_redraw(lambda : Polygon(circle.get_top(), circle.get_right(), circle.get_left(),
                                                  fill_color = GREEN_C))
