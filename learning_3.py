...
class Matrix(LinearTransformationScene):
    ...
    def construct(self):
        ...
        circ1 = Circle(radius = 3, stroke_color = BLUE_A,
                       fill_color = BLUE_D, fill_opacity = 0.6).shift(DOWN*2+RIGHT*1)
...
class Example(Scene):
    def construct(self):
        ...
        vect1 = Line(start = plane.coords_to_point(0,0), end = plane.coords_to_point(3,2), stroke_color = RED).add_tip()
        ...
        self.add(box)
        self.wait()
        self.play(stuff.animate.move_to(box.get_center()).set(width = 3.2), run_time =3)
        self.wait()
